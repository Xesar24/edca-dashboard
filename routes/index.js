...
router.get('/d3-bubble-chart-data', function (req, res) {
    edca_db.manyOrNone("select ( select string_agg(identifier_id, '; ') from supplier where contractingprocess_id = contractingprocess.id) as identifier_id, (select string_agg(identifier_legalname, '; ') from supplier where contractingprocess_id=contractingprocess.id) as identifier_legalname, ocid, contract.title,tender.procurementmethod, "+
    ...
    "and (contract.period_enddate is not null and contract.period_startdate is not null) order by cast (  ( DATE_PART('year', period_enddate) - DATE_PART('year', period_startdate) ) * 12  + ( DATE_PART('month', period_enddate) - DATE_PART('month', period_startdate))  as integer) / 12").then(function (data) {
        res.json(data);
    }).catch(function (error) {
        res.json(error);
    });
